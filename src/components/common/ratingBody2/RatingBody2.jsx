                ...
                { _.map(filterRatings, (rating, i) =>
                    (
                        <li className="rating2-item border-1px" key={i}>
                            ...
                            <div className="desc">
                                <div className="info clearfix"><span className="name">{rating.username}</span><span
                                    className="time">{moment(rating.rateTime).format('YYYY-MM-DD HH:mm')}</span></div>
                                <div className="level">
                                    <Star size={24} score={rating.score}
                                          style={{display: 'inline-block'}}/>{rating.deliveryTime && <span
